...
Parser.prototype._transform = function(file, encoding, done) {
	...
	while (( matches = functionRegex.exec( fileContent ) )) {
		...
		if (key) {
			//take string after ${ and before } to get varaible value
			let patternVar = /{(.*?)}/;
			var variableRegex = new RegExp( patternVar, 'g' );
			let matchesVar = variableRegex.exec(key);
			let searchVar = matchesVar[1];

			//search for const variable = value and retrieve value
			var e = new RegExp("const " + searchVar + "='(\\S*)'" , 'g')
			const value = e.exec(fileContent)[1];

			// replace searchVar by value
			key = key.replace('${' + searchVar +'}', value );
